...
public final @Internal class AdvancementStatWidget extends TClickableWidget
{
	...
	// ==================================================
	public AdvancementStatWidget(int x, int y, PlacedAdvancement advancement) throws NullPointerException
	{
		super(x, y, SIZE, SIZE);
		this.advancement = Objects.requireNonNull(advancement);
		
		final var tooltip = literal("");
		final @Nullable var display = advancement.getAdvancement().display().orElse(null);
		if(display != null)
		{
		...
		{
			tooltip.append(literal(advancement.getAdvancementEntry().id().toString()).formatted(Formatting.YELLOW));
	 ...
	 */
	public final PlacedAdvancement getAdvancement() { return this.advancement; }
	// ==================================================
