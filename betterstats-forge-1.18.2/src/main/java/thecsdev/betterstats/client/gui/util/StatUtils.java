...
public class StatUtils
{
	...
	// ==================================================
	public static ArrayList<SUGeneralStat> getGeneralStats(StatsCounter statHandler)
	{
		//create a new list
	    ...
	    {
	    	//get next stat
			Stat<ResourceLocation> stat = statListIterator.next();
			int val = statHandler.getValue(stat);
	...
	// --------------------------------------------------
	public static LinkedHashMap<ItemGroup, ArrayList<SUItemStat>> getItemStats(StatsCounter statHandler, String searchQuery)
	{
	...
	{
		//get the forge loader mod
		ModFileInfo mfi = FMLLoader.getLoadingModList().getModFileById(modId);
		
		//check if the mod exists and return the mod display name
		if(mfi != null) return mfi.moduleName();
		else return modId;
	}
	// ==================================================
	...
	public static class SUGeneralStat
	{
		...
		public final Component txtValue;
		public final int intValue;
		
		public SUGeneralStat(Component title, Component value, int intValue)
		{
			this.title = title;
			this.txtValue = value;
	...
	public static class SUMobStat
	{
		...
		public SUMobStat(StatCounter statHandler, EntityType<?> entityType)
		{
			...
			this.killed = statHandler.getStat(Stats.ENTITY_KILLED, entityType);
			this.killedBy = statHandler.getStat(Stats.ENTITY_KILLED_BY, entityType);
		}
		
		public Entity safelyCreateEntity(EntityType<?> et)
		{
			if(et == null) return EntityType.MARKER.create(MCClient.level);
			try { return et.create(MCClient.level); }
			catch(Exception exc)
			{
				if(!IGNORE_ENTITY_RENDER_ERRORS) throw exc;
				errored = true;
				return EntityType.MARKER.create(MCClient.level);
			}
		}
