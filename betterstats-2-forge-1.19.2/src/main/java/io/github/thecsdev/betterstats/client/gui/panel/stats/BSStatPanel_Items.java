...
public class BSStatPanel_Items extends BSStatPanel
{
	...
	public void init(StatsCounter statHandler, Predicate<StatUtilsStat> statFilter)
	{
		...
		for(var iGroup : itemStats.keySet())
		{
			Component gLabel = iGroup != null ? iGroup.getDisplayName() : null;
	...
	protected class BSStatWidget_Item extends BSStatWidget
	{
		...
		public BSStatWidget_Item(StatUtilsItemStat stat, int x, int y)
		{
			super(x, y, SIZE, SIZE);
			this.stat = stat;
			this.stack = stat.item.getDefaultInstance();
		...
		public void updateTooltip()
		{
			...
			Component tooltip = literal(
					(SHOW_ITEM_NAMES ? translatable(stat.item.getDescriptionId()).getString() + "\n\n" : "") +
					sMined + " - " + stat.sMined + "\n" +
		...
		{
			//handle REI - REI broken, find a way to fix this
			/*if((button == 0 || button == 1) && this.stack != null && !Screen.hasShiftDown())
			try
			{
				//create a new ViewSearchBuilder
				var builder = me.shedaniel.rei.api.client.view.ViewSearchBuilder.builder();
				
				//get entry stack
				var entryStack = me.shedaniel.rei.api.common.util.EntryStacks.of(this.stack);
			...
			{
				String url = BetterStatsRegistry.getItemWikiURL(ForgeRegistries.ITEMS.getKey(this.stat.item));
				if(url != null)
				{
		...
		public void render(PoseStack matrices, int mouseX, int mouseY, float deltaTime)
		{
			...
			//render the item
			getItemRenderer().renderGuiItem(this.stack, getTpeX() + 3, getTpeY() + 3);
		}
		// ----------------------------------------------
		/*@Override - TODO - fix it please :(
		protected void onContextMenu(TContextMenuPanel contextMenu)
		{
			...
			contextMenu.addButton(translatable("betterstats.gui.ctx_menu.pin_to_hud"), btn ->
			{
				...
				bshs.addHudStatWidget(new BSHudStatWidget_Item(0, 0, stat.statsCounter, stat.item));
			});
