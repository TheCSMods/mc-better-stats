...
public final class StatisticsCommand
{
	...
	private static SuggestionProvider<ServerCommandSource> SUGGEST_STAT = (context, builder) ->
	{
		...
		try { statType = RegistryEntryReferenceArgumentType.getRegistryEntry(context, "stat_type", RegistryKeys.STAT_TYPE).value(); }
		catch(Exception e) {}
	...
	private static int execute_edit(CommandContext<ServerCommandSource> context, boolean setOrIncrease)
	{
		try
		{
			...
			final var stat_object = arg_stat_type.getRegistry().getOptionalValue(arg_stat).orElse(null);
